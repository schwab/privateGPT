...
# Clone schwab/privateGPT
#RUN git clone https://github.com/schwab/privateGPT.git /src
...
ENV EXTRA_LAUNCH_ARGS="--n-gpu-layers 20000"
...
#ENV EXTRA_LAUNCH_ARGS="--n-gpu-layers 20000"
#CMD ["python3", "/app/server.py --n-gpu-layers 20000"]
#CMD ["streamlit", "run", "/app/server.py"]
WORKDIR /app
CMD ["streamlit", "run", "server.py", "--server.port=7860", "--server.address=0.0.0.0"]
#CMD ["/bin/bash"]
...
# ENV CLI_ARGS "--n-gpu-layers 20000"
# CMD ["python3", "/app/server.py ${CLI_ARGS}"]
